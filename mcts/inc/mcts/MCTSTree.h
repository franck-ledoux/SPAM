...
public:
   ...
   MCTSTree* get_parent() const;
     ...
     */
    MCTSTree* get_child(const IAction* AAction) const;

    MCTSTree* get_most_visited_child() const;


    void add_child(MCTSTree* AChild);
     ...
     *        based on the exploration/exploitation principle (UCT criterion here)
       ...
       */
    const IAction* select_action_UCT(double AC=1.41) const;
    /**@brief Among all the possible action to try, provide one to apply randomly
     * @return the selected action
     */
    IAction* random_action() const;
...
private:
    ...
    std::vector<IAction*> m_untried_actions;
};
/*---------------------------------------------------------------------------*/
